				...
				int boneMask )
{
	...
	if ( seqdesc.flags & STUDIO_DELTA )
	{
		...
		for ( i = 0; i < nBoneCountRoundedFour; i+=4 )
		{
			...
			{
				i32x4 tinyScales = CmpLeSIMD( weightfour, Four_Zeros );
			...
			*/
			StoreUnaligned3SIMD( pos1[i + 0].Base(), MaskedAssign( ( i32x4 ) splatweights[0], pos1simd[0], originalpos1simd[0] ) );
			StoreUnaligned3SIMD( pos1[i + 1].Base(), MaskedAssign( ( i32x4 ) splatweights[1], pos1simd[1], originalpos1simd[1] ) );
			StoreUnaligned3SIMD( pos1[i + 2].Base(), MaskedAssign( ( i32x4 ) splatweights[2], pos1simd[2], originalpos1simd[2] ) );
			StoreUnaligned3SIMD( pos1[i + 3].Base(), MaskedAssign( ( i32x4 ) splatweights[3], pos1simd[3], originalpos1simd[3] ) );
...
FORCEINLINE fltx4 BoneQuaternionAlignSIMD( const fltx4 &p, const fltx4 &q )
{
	...
	i32x4 cmp = CmpLtSIMD( Dot4SIMD(p,q), Four_Zeros );
