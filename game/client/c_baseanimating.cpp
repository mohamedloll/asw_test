...
void C_BaseAnimating::LockStudioHdr()
{
	...
	if (mdl)
	{
		...
		if ( m_hStudioHdr != MDLHANDLE_INVALID )
		{
			...
			{
				MDLHandle_t hVirtualModel = VoidPtrToMDLHandle(pStudioHdrContainer->GetRenderHdr()->VirtualModel());				mdlcache->LockStudioHdr( hVirtualModel );
			}
...
#endif
		{
			m_pRagdoll->RagdollBone( this, const_cast<mstudiobone_t*>(pbones), hdr->numbones(), boneSimulated, m_BoneAccessor );
		}
