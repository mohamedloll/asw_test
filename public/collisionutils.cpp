...
bool IsAnyTrue(const fltx4& a)							// (a.x < 0) || (a.y < 0) || (a.z < 0) || (a.w < 0)
{
	...
	const Vector& origin, const Vector& vecDelta, float flTolerance)
{
	...
	i32x4 startOutMins = CmpLtSIMD(Four_Zeros, offsetMinsExpanded);
	i32x4 endOutMins = CmpLtSIMD(delta, offsetMinsExpanded);
	i32x4 minsMask = AndSIMD(startOutMins, endOutMins);
	i32x4 startOutMaxs = CmpGtSIMD(Four_Zeros, offsetMaxsExpanded);
	i32x4 endOutMaxs = CmpGtSIMD(delta, offsetMaxsExpanded);
	i32x4 maxsMask = AndSIMD(startOutMaxs, endOutMaxs);
	...
	i32x4 crossPlane = OrSIMD(XorSIMD(startOutMins, endOutMins), XorSIMD(startOutMaxs, endOutMaxs));
	...
	i32x4 separation = CmpGtSIMD(lastIn, firstOut);
	...
	const Vector& vecInvDelta, float flTolerance)
{
	...
	// Check to see if both the origin (start point) and the end point (delta) are on the front side
	// of any of the box sides - if so there can be no intersection
	i32x4 startOutMins = CmpLtSIMD(Four_Zeros, boxMins);
	i32x4 endOutMins = CmpLtSIMD(delta, boxMins);
	i32x4 minsMask = AndSIMD(startOutMins, endOutMins);
	i32x4 startOutMaxs = CmpGtSIMD(Four_Zeros, boxMaxs);
	i32x4 endOutMaxs = CmpGtSIMD(delta, boxMaxs);
	i32x4 maxsMask = AndSIMD(startOutMaxs, endOutMaxs);
	if (IsAnyTrue(SetWToZeroSIMD(OrSIMD(minsMask, maxsMask))))
		return false;
...
#endif
{
	...
	// of any of the box sides - if so there can be no intersection
	i32x4 startOutMins = AndSIMD(CmpLtSIMD(Four_Zeros, boxMin), CmpLtSIMD(delta, boxMin));
	i32x4 startOutMaxs = AndSIMD(CmpGtSIMD(Four_Zeros, boxMax), CmpGtSIMD(delta, boxMax));
