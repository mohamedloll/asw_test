...
#include <windows.h>
#endif
#include "../../common/dx11sdk/xaudio2.h"
#include "mix.h"
...
int Audio_EnumerateXAudio2Devices(audio_device_description_t* pDeviceListOut, int nListCount)
{
	...
	UINT nDeviceCountWindows = 0;
	pCollection->GetCount(&nDeviceCountWindows);

	int nIterateCount = (int)std::min<UINT>(nListCount, nDeviceCountWindows);
	...
	for (UINT i = 0; i < (UINT)nIterateCount; ++i)
	{
		...
		{
			V_wcscpy_safe(pDeviceListOut[i].m_deviceName, pwszId);
			CoTaskMemFree(pwszId);
		}
		else
		{
			pDeviceListOut[i].m_deviceName[0] = 0;
		}
		...
		if (SUCCEEDED(hr) && pProps)
		{
			...
			{
				// convert to char (approx); keep safe buffer sizes
				int cb = WideCharToMultiByte(CP_UTF8, 0, varName.pwszVal, -1, pDeviceListOut[i].m_friendlyName, sizeof(pDeviceListOut[i].m_friendlyName), NULL, NULL);
				(void)cb;
			}
		...
		{
			pDeviceListOut[i].m_friendlyName[0] = 0;
		}

		// channel count via IAudioClient::GetMixFormat
		UINT32 channels = 2;
		...
		pDeviceListOut[i].m_nChannelCount = (uint8)channels;
		...
		if (pDefaultDevice)
		{
			...
			if (SUCCEEDED(pDefaultDevice->GetId(&pDefaultId)) && pDefaultId)
			{
				if (V_wcscmp(pDeviceListOut[i].m_deviceName, pDefaultId) == 0)
				{
		...
		// track max-channel device index
		if (nMaxChannelDevice < 0 || pDeviceListOut[i].m_nChannelCount > pDeviceListOut[nMaxChannelDevice].m_nChannelCount)
		{
			nMaxChannelDevice = (int)i;
		}
	...
	// If no default found, mark device with maximum channels default
...
IAudioDevice2 *Audio_CreateXAudio2Device( const audio_device_init_params_t &params )
{
	...
	const int kMaxXAudio2Devices = 64;
	desc.SetCount(kMaxXAudio2Devices);
	nCount = Audio_EnumerateXAudio2Devices(desc.Base(), desc.Count());

	// Trim vector to the real count
	if (nCount >= 0 && nCount < desc.Count())
	{
		desc.SetCount(nCount);
	}
