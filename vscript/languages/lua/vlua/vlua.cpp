...
//		m_TypeMap.Init( 256 );
	...
	static void PushVariant( lua_State *pState, const ScriptVariant_t &value )
	{
		switch ( value.m_type )
		{
			...
			case FIELD_HSCRIPT:		
				...
				{
					lua_rawgeti( pState, LUA_REGISTRYINDEX, static_cast< int > ( ( intp )value.m_hScript ) );
	...
	static void ReleaseVariant( lua_State *pState, ScriptVariant_t &value )
	{
		...
		{
			luaL_unref( pState, LUA_REGISTRYINDEX, static_cast< int >( ( intp )value.m_hScript ) );
		}
		else
		{
			value.Free();
...
//			lua_call( m_LuaState, 0, 0 );
	...
	{
		luaL_unref( m_LuaState, LUA_REGISTRYINDEX, static_cast< int > ( ( intp )hScript ) );
	}

	virtual ScriptStatus_t Run( HSCRIPT hScript, HSCRIPT hScope = NULL, bool bWait = true )
	{
		lua_rawgeti( m_LuaState, LUA_REGISTRYINDEX, static_cast< int >( ( intp )hScript ) );
	...
	virtual HSCRIPT LookupFunction( const char *pszFunction, HSCRIPT hScope = NULL )
	{
		if ( hScope )
		{
			lua_rawgeti( m_LuaState, LUA_REGISTRYINDEX, static_cast< int >( ( intp )hScope ) );
	...
	virtual ScriptStatus_t ExecuteFunction( HSCRIPT hFunction, ScriptVariant_t *pArgs, int nArgs, ScriptVariant_t *pReturn, HSCRIPT hScope, bool bWait )
	{
		...
		try
		{
			if ( hFunction )
			{
				...
				lua_rawgeti( m_LuaState, LUA_REGISTRYINDEX, static_cast< int >( ( intp )hFunction ) );
...
//				params[i].Free();
	...
	void RegisterFunctionGuts( ScriptFunctionBinding_t *pScriptFunction, HSCRIPT pOwningClass = NULL )
	{
		...
		if ( pOwningClass )
		{
			lua_rawgeti( m_LuaState, LUA_REGISTRYINDEX, static_cast< int >( ( intp )pOwningClass ) );
			if ( !lua_isnil( m_LuaState, -1 ) )
			{
...
//		lua_setfield( m_LuaState, -2, "__gc" );
		...
		luaL_unref( m_LuaState, LUA_REGISTRYINDEX, static_cast< int >( ( intp )ClassReference ) );
